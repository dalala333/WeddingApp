...
public class PaymentActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.payment_management);
    ...
    private void viewItemList() {
        try {
            ...
            String query = "SELECT * FROM PaymentActivity WHERE event_id = ?";
    ...
    private void addItem() {
        try {
            ...
            long result = db.insert("PaymentActivity", null, values);
            if (result == -1) {
    ...
    private void editItem() {
        try {
            ...
            int rows = db.update("PaymentActivity", values, "id = ?", new String[]{itemIdInput.getText().toString()});
            if (rows > 0) {
    ...
    private void deleteItem() {
        try {
            SQLiteDatabase db = dbHelper.getWritableDatabase();
            int rows = db.delete("PaymentActivity", "id = ?", new String[]{itemIdInput.getText().toString()});
            if (rows > 0) {
