...
public class GuestActivity extends AppCompatActivity {
    ...
    private void viewItemList() {
        try {
            ...
            String query = "SELECT * FROM GuestActivity WHERE event_id = ?";
    ...
    private void addItem() {
        try {
            ...
            long result = db.insert("GuestActivity", null, values);
            if (result == -1) {
    ...
    private void editItem() {
        try {
            ...
            int rows = db.update("GuestActivity", values, "id = ?", new String[]{itemIdInput.getText().toString()});
            if (rows > 0) {
    ...
    private void deleteItem() {
        try {
            SQLiteDatabase db = dbHelper.getWritableDatabase();
            int rows = db.delete("GuestActivity", "id = ?", new String[]{itemIdInput.getText().toString()});
            if (rows > 0) {
